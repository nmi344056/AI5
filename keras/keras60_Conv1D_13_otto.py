...
#2. 모델 구성
model = Sequential()
model.add(Conv1D(filters=64, kernel_size=3, input_shape=(93,1), activation='relu'))
...
filepath = "".join([path, 'k59_13_otto_diabetes_date_', date, '_epo_', filename])
...
start = time.time()
hist = model.fit(x_train, y_train, epochs=100, batch_size=60, validation_split=0.2, callbacks=[es, mcp])
end = time.time()
...
LSTM
loss : [3.4132847785949707, 0.26050421595573425]
time : 19070.88 초

Conv1D
loss : [1.4251174926757812, 0.26696833968162537]
time : 205.49 초
