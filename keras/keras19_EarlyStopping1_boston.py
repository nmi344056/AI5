...
es = EarlyStopping(
    monitor='val_loss',
    mode = 'min',               # 모르면 auto 최대면 max
    patience=10,                # 10번 참고 더 해본다
    restore_best_weights=True,   # 이놈을 안쓰면 patience 지점이 된다.
)
hist = model.fit(x_train, y_train, epochs=1000, batch_size=32,
          validation_split=0.2,
          callbacks=[es],     #대괄호는 리스트, 지금은 es하나지만 나중에 친구들 나오겠구나
          )
end = time.time()
