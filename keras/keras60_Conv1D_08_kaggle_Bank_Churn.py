...
# #2. 모델구성
model = Sequential()
model.add(Conv1D(filters=64, kernel_size=1, input_shape=(2,5), activation='relu'))
model.add(Conv1D(64, 1))
# model.add(Conv2D(64, (3,3), activation='relu', input_shape=(2, 5, 1), strides=1, padding='same'))
model.add(Flatten())
...
filepath = "".join([path, 'k59_08_date_', date, '_epo_', filename])
...
test_csv = test_csv.reshape(110023, 2, 5, 1)
...
LSTM
loss :  0.32847869396209717
accuracy :  0.86
time :  4094.59 초

Conv1D
loss :  0.3426096737384796
accuracy :  0.854
time :  3002.58 초 (생각보다는 빠르지 않았다)
