...
from tensorflow.keras.layers import SimpleRNN, LSTM, GRU, Conv1D, Conv2D, Flatten, Dropout
...
x_n = NAVER_csv.drop(['전일비', 'Unnamed: 6'], axis=1)
x_h = HYBE_csv.drop(['전일비', 'Unnamed: 6'], axis=1)
y_u = seongu_csv['종가']
print(x_n.shape, x_h.shape, y_u.shape)          # (948, 14) (948, 14) (948,)
...
print(x_n.shape, x_h.shape)                     # (929, 20, 14) (929, 20, 14)

xn_predict = x_n[:1]
xh_predict = x_h[:1]
print(xn_predict.shape, xh_predict.shape)       # (1, 20, 14) (1, 20, 14)
...
# dense12 = Conv1D(64, kernel_size=3, activation='relu', strides=1, padding='same')(drop11)
# drop12 = Dropout(0.2)(dense12)
# flatten = (Flatten())(drop12)
...
# start = time.time()
# model.fit([xn_train, xh_train], yu_train, epochs=1000, batch_size=32, validation_split=0.2, verbose=3, callbacks=[es, mcp])
# end = time.time()

#4. 평가, 예측
path_w = 'C:\\ai5\\_save\\중간고사가중치\\'
model = load_model(path_w + 'k63_01_date_0816.1646_epo_0354_valloss_349296.9062.hdf5')
