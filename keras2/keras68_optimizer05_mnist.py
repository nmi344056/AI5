...
for i in range(len(lr)):
    ...
    filepath = "".join([path, 'k68_05_date_', str(i+1), '_', date, '_epo_', filename])
    ...
    start = time.time()
    hist = model.fit(x_train, y_train, epochs=500, batch_size=16, validation_split=0.2, verbose=1, callbacks=[es, mcp])
    end = time.time()
