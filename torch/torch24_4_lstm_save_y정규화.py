...
class Custom_Dataset(Dataset):
    def __init__(self):
        ...
        self.y = (self.y - np.min(self.y, axis=0)) / (np.max(self.y, axis=0) - np.min(self.y, axis=0))  # 전체 데이터 정규화
...
with torch.no_grad():
    model.load_state_dict(torch.load(save_path + 't24_3.pth', map_location=DEVICE))
...
'''
R2: 0.9720229257542672
total_loss: 0.002673822222277522
