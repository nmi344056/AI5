...
x1 = torch.FloatTensor(x).unsqueeze(1).to(DEVICE)    # .to(DEVICE) 추가, = to torch.device('cuda')
print('스케일링 전 :', x)
x = (x1 - torch.mean(x1)) / torch.std(x1)              # StandardScaler
print('스케일링 후 :', x)
'''
스케일링 전 : tensor([[1.],    
        [2.],
...
x_predict = (torch.Tensor([[4]]).to(DEVICE) - torch.mean(x1)) / torch.std(x1)

y = torch.FloatTensor(y).unsqueeze(1).to(DEVICE)    # .to(DEVICE) 추가
...
model = nn.Linear(1, 1).to(DEVICE)    # .to(DEVICE) 추가     # (인풋, 아웃풋), y = xw + b
...
# results = model(torch.Tensor([[4]]).to(DEVICE))    # .to(DEVICE) 추가
...
'''
epoch: 2000, loss : 1.1227760041143675e-11
==============================
